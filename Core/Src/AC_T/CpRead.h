...
#define EVSE_CP_ADC_BUF_LEN  7692 // SampleRate/5, a little less than 100ms per half (99.996ms)
 ...
 *   same as Zerova logs
 *
 *  Updated every cpADCdataProcess(~100ms), but
 *
 */
extern volatile uint8_t evse_cp_prev_read_state; //previous 100ms state
extern volatile uint8_t evse_cp_cur_read_state; //latest 100ms state
